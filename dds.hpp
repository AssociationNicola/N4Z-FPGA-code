...
class Dds
{
  public:
    ...
    void set_dds_freq(double freq_hz) {
        ...
        ctx.log<INFO>("fs %lf ,  n4z rx frequency set to %lf \n", double(prm::adc_clk), freq_hz);
    }

    void set_msf_dds_freq(double freq_hz) {
        ...
        ctl.write_reg<uint64_t>(reg::msf_phase_incr0, uint64_t(factor * freq_hz));
        msf_dds_freq = freq_hz;

        ctx.log<INFO>("fs %lf ,  msf. frequency set to %lf \n", double(prm::adc_clk), freq_hz);
    }
